...
class Confirm extends Component {
    ...
    Confirm() {
        ...
        fetch(`https://app-a-store.herokuapp.com/api/diterima/${produk.id}`, {
            method: 'POST',
    ...
    render() {
        ...
        return (
            <View style={{ flex: 1 }}>
                ...
                {produk.status == "pengiriman" ? (
                    <TouchableOpacity onPress={() => this.Confirm()} style={style.button}>
                        {this.state.loading ? (<LottieView source={require("../assets/890-loading-animation.json")} style={{ height: 70, width: 50, marginTop: 2 }} autoPlay loop />
                                ...
                                <Text style={{ color: "#ff6000" }}>Konfirmasi Barang Sampai</Text>
                            )}
                    </TouchableOpacity>
                ) : (
